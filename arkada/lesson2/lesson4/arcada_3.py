""" Управление стрелкой вниз убираем. Прыгать вверх можно только если стоим на опоре!
для этого: свойство stand_on (опора, на которой стоим).
Свойство меняется в update (строки 81-84, 86), используется в методе jump() - стр. 56 - 58
Управление по стрелке вверх вызывает jump(), стр. 175
"""
...
while run:
    ...
    bombs.draw(
        window)  # группу бомб рисуем отдельно - так бомба, которая ушла из своей группы, автоматически перестанет быть видимой!
