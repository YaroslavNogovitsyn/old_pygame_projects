''' Добавляем гравитацию - метод gravitate(), стр. 49-50. 
Вызываем каждый раз внутри update (строка 66). Почему так резко падает?
'''
    ...
    def update(self):
        ...
        if self.x_speed > 0: # идем направо, правый край персонажа - вплотную к левому краю стены
            for p in platforms_touched:
                self.rect.right = min(self.rect.right, p.rect.left) # если коснулись сразу нескольких, то правый край
                # - минимальный из возможных
        ...
        if self.y_speed > 0: # идем вниз
            for p in platforms_touched:
                self.rect.bottom = min(self.rect.bottom, p.rect.top) # ставим нижний край вплотную к самому верхнему из верхних краев стен
